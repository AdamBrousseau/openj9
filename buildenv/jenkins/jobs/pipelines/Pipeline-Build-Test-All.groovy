...
CURRENT_RELEASES = ['11', '8', '12', '13', 'next']
    ...
    for (spec in BUILD_SPECS.keySet().sort()){
        ...
        buildReleases.each { release ->
            ...
            if (BUILD_SPECS.get(spec).contains(release)) {
                pipelineName = get_pipeline_name(spec, release)
                build = pipelineBuilds.get(pipelineName)
                ...
                if (build) {
                    ...
                    pipelineRuntime = build.getDurationString()
                }
            }

            innerTable += "<tr><td>&nbsp;</td><td style=\"text-align: right;\">${pipelineLink}</td><td style=\"text-align: right;\">${pipelineRuntime}</td></tr>"
            ...
            downstreamJobNames.each { label, jobName ->
                ...
                innerTable += "<tr style=\"vertical-align: bottom;\"><td>${aLabel}</td><td style=\"text-align: right\">${link}</td><td style=\"text-align: right\">${runtime}</td></tr>"
            }

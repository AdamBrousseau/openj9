...
def cleanupBuilds(artifactory_server, artifactory_repo, jobToCheck, artifactory_num_artifacts, artifactoryCreds) {
    ...
    stage('Discover Stored Artifacts') {
        ...
        def request = httpRequest authentication: artifactoryCreds, consoleLogResponseBody: true, validResponseCodes: '200,404', url: "${artifactory_server}/api/storage/${artifactory_repo}${internalSubfolder}${testSubfolder}/${jobToCheck}"
    ...
    stage('Delete Old Artifacts') {
        ...
        if (amount_deleted > 0) {
            ...
            for (i = 0; i < amount_deleted; i++) {
                ...
                if ("${params.DRY_RUN}" == "false") {
                    httpRequest authentication: artifactoryCreds, httpMode: 'DELETE', consoleLogResponseBody: true, url: "${artifactory_server}/${artifactory_repo}${internalSubfolder}${testSubfolder}/${jobToCheck}/${folderNames[i]}"
                }
            }
        } else {
        ...
        if (artifactory_max_num_artifacts == 0) {
            ...
            if ("${params.DRY_RUN}" == "false") {
                httpRequest authentication: artifactoryCreds, httpMode: 'DELETE', consoleLogResponseBody: true, url: "${artifactory_server}/${artifactory_repo}${internalSubfolder}${testSubfolder}/${jobToCheck}"
            }
        }
...
def cleanupTime(artifactory_server, artifactory_repo , artifactory_days_to_keep_artifacts, artifactoryCreds) {
    stage('Discover Old Artifacts') {
        ...
        if ("${params.DRY_RUN}" == "true") {
            currentBuild.description = "Dry Run<br>"
        } else {
            currentBuild.description = ""
        }
        currentBuild.description += "Deleting build over ${artifactory_days_to_keep_artifacts} days"

        def request = httpRequest authentication: artifactoryCreds, consoleLogResponseBody: true, validResponseCodes: '200,404', url: "${artifactory_server}/api/search/usage?notUsedSince=${current_time}&createdBefore=${created_before_time}&repos=${artifactory_repo}&artifactory.search.userQueryLimit"
        data = readJSON text: request.getContent()
        requestStatus = request.getStatus()
    }
    stage ('Delete Old Artifacts') {
